...
const article = (slug, date, reviewedHash = 'reviewed') => ({ slug, publishAt: `${date}T09:00:00+08:00`, reviewedHash, article: `content/${slug}.md`, files: [`content/${slug}.md`] });
const plan = { articles: [article('one', '2026-09-22'), article('two', '2026-09-23')] };
test('Beijing 09:00 boundary is independent of the machine timezone', () => {
  assert.equal(releasedArticles(plan, new Date('2026-09-23T00:59:59Z')).length, 1);
  assert.equal(releasedArticles(plan, new Date('2026-09-23T01:00:00Z')).length, 2);
